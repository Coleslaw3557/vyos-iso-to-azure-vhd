...
if [[ "$VYOS_RELEASE" == "equuleus" ]]; then
    debian_list_url="http://${PACKER_HTTP_ADDR}/debian_11-vyos.list"
elif [[ "$VYOS_RELEASE" == "current" || "$VYOS_RELEASE" == "sagitta" || "$VYOS_RELEASE" == "circinus" ]]; then
    debian_list_url="http://${PACKER_HTTP_ADDR}/debian_12-vyos.list"
else
...
wget --timeout=30 --tries=3 --retry-connrefused -O "$tmp_file" "$debian_list_url" || { echo "cant download debian.list from packer http repo"; exit 1; }
